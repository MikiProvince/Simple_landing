...
const FEATURES_LIST = document.querySelectorAll('.features__item');
const FEATURES_P = document.querySelector('.features__p');
let txt = ' ';
const HERO_TITLES = ['Разработано 2000+ приложений','Бесплатные приложения','Тысячи довольных клиентов']
const HERO_SUBTITLES = ['Для вашего удобства мы разработали множество приложений для вашего бизнеса и команды, которые доступны в любое время суток','К любому сервисному обслуживанию мы подключим советы по управлению компанией совершенно бесплатно','Накопленный нами опыт позволяет предлагать клиентам обширный спектр качественных приложений и сервисов'] 
const FEATURES_P_ARR = [
...
HERO_DOTS.forEach((dot,index) => dot.addEventListener('click', event =>{
    ...
    HERO_IMAGE.src = `assets/hero${index + 1}.jpg`;
